...
function values(formData: FormData) {
  ...
  return { direction, departureAt: departureAt.toISOString(), carModel, price, notes, stops, startingCity: direction === "berlin-zagreb" ? "Berlin" : "Zagreb", destinationCity: direction === "berlin-zagreb" ? "Zagreb" : "Berlin" } as const;
}
...
export async function createTrip(formData: FormData) {
  ...
  const { data: trip, error } = await supabase.from("trips").insert({ driver_id: user.id, direction: parsed.direction, departure_at: parsed.departureAt, car_model: parsed.carModel, price: parsed.price, starting_city: parsed.startingCity, destination_city: parsed.destinationCity, notes: parsed.notes }).select("id").single();
...
export async function updateTrip(formData: FormData) {
  ...
  const { error } = await supabase.from("trips").update({ direction: parsed.direction, departure_at: parsed.departureAt, car_model: parsed.carModel, price: parsed.price, starting_city: parsed.startingCity, destination_city: parsed.destinationCity, notes: parsed.notes }).eq("id", id).eq("driver_id", user.id);
