...
function values(formData: FormData) {
  ...
  const zagrebSideEndpoint = String(formData.get("zagrebSideEndpoint") ?? "").trim();
  ...
  if (zagrebSideEndpoint.length > 100) return { error: "The additional starting point or destination must contain no more than 100 characters." } as const;
  return { direction, departureAt: departureAt.toISOString(), ...flexibility, carModel, price, petFriendly, notes, stops, zagrebSideEndpoint: zagrebSideEndpoint || null, startingCity: direction === "berlin-zagreb" ? "Berlin" : "Zagreb", destinationCity: direction === "berlin-zagreb" ? "Zagreb" : "Berlin" } as const;
}
...
export async function updateTrip(formData: FormData) {
  ...
  const { error } = await supabase.from("trips").update({ direction: parsed.direction, departure_at: parsed.departureAt, flexibility_type: parsed.flexibilityType, date_flexibility: parsed.dateFlexibility, time_flexibility: parsed.timeFlexibility, car_model: parsed.carModel, price: parsed.price, pet_friendly: parsed.petFriendly, starting_city: parsed.startingCity, destination_city: parsed.destinationCity, zagreb_side_endpoint: parsed.zagrebSideEndpoint, notes: parsed.notes }).eq("id", id).eq("driver_id", user.id);
