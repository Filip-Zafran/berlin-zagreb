...
export default async function EditTripPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const { data: trip } = await supabase.from("trips").select("id, direction, departure_at, car_model, price, notes, trip_stops(city, position)").eq("id", id).eq("driver_id", user.id).single();
