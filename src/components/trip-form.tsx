...
type InitialTrip = { id?: string; direction?: string; zagrebSideEndpoint?: string; date?: string; time?: string; flexibilityType?: FlexibilityType; dateFlexibility?: DateFlexibility; timeFlexibility?: TimeFlexibility; carModel?: string; price?: string; petFriendly?: boolean; stops?: string[]; notes?: string };
...
export function TripForm({ action, initial = {} }: { action: (data: FormData) => void | Promise<void>; initial?: InitialTrip }) {
  ...
  const [hasDifferentEndpoint, setHasDifferentEndpoint] = useState(Boolean(initial.zagrebSideEndpoint));
  ...
  return <form action={action} className="mt-8 space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
    ...
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
      <label className="flex cursor-pointer items-start gap-3 text-sm font-bold text-slate-700"><input type="checkbox" checked={hasDifferentEndpoint} onChange={(event) => setHasDifferentEndpoint(event.target.checked)} className="mt-0.5 size-5 rounded border-slate-300 accent-slate-900" /><span>{direction === "zagreb-berlin" ? "Start before Zagreb" : "Continue beyond Zagreb"}<span className="mt-1 block text-xs font-normal leading-5 text-slate-500">Berlin stays fixed and the trip must still pass through Zagreb.</span></span></label>
      {hasDifferentEndpoint && <label className="mt-4 block text-sm font-bold text-slate-700">{direction === "zagreb-berlin" ? "Starting point" : "Final destination"}<input className={inputClass} name="zagrebSideEndpoint" required maxLength={100} defaultValue={initial.zagrebSideEndpoint} placeholder={direction === "zagreb-berlin" ? "Split, Rijeka…" : "Split, Rijeka…"} /></label>}
    </div>
    ...
    <div><p className="text-sm font-bold text-slate-700">Route</p><p className="mt-2 rounded-xl bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-600">{direction === "berlin-zagreb" ? <>Berlin → planned stops → Zagreb{hasDifferentEndpoint && " → final destination"}</> : <>{hasDifferentEndpoint && "starting point → "}Zagreb → planned stops → Berlin</>}</p></div>
